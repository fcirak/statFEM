...
//! @file RestartWriteRead.hpp

// Note:
// Identical with gshell/fem/RestartWriteRead.hpp

#ifndef beam_fem_restartwriteread_h
#define beam_fem_restartwriteread_h

#include <stdio.h>
#include <string>

#include <fsi/utils/Writers.hpp>
#include <fsi/utils/Readers.hpp>
...
//------------------------------------------------------------------------------
template<typename DRIVER>
void beam::fem::restartWrite( const std::string & basename,
                              const unsigned step,
                              DRIVER & driver )
{
    ...
    beam::fem::restartWriteData( disp, getDisp, driver  );
    disp.close();
    ...
    beam::fem::restartWriteData( vel, getVel, driver );
    vel.close();
    ...
    beam::fem::restartWriteData( acc, getAcc, driver );
    acc.close();
...
//------------------------------------------------------------------------------
template< typename GETFUN, typename DRIVER >
void beam::fem::restartWriteData( std::ostream & out, GETFUN & getFun,
                                  DRIVER & driver )
{
    typedef typename DRIVER::Node Node;
...
//------------------------------------------------------------------------------
template<typename DRIVER>
void beam::fem::restartRead( const std::string & basename,
                             const unsigned step,
                             DRIVER & driver )
{
    typedef typename DRIVER::Node Node;
    ...
    SetFun setDisp = std::bind( &Node::setDisplacements, std::placeholders::_1, std::placeholders::_2 );
    beam::fem::restartReadData( disp, setDisp, driver );
    disp.close();
        
    // write veloc
    const std::string velFileName = basename + "_V_" + s + ".dat";
    std::ifstream vel( velFileName.c_str() );
    SetFun setVel = std::bind( &Node::setVelocities, std::placeholders::_1, std::placeholders::_2 );
    beam::fem::restartReadData( vel, setVel, driver );
    vel.close();

    // write accel
    const std::string accFileName = basename + "_A_"+ s + ".dat";
    std::ifstream acc( accFileName.c_str() );
    SetFun setAcc = std::bind( &Node::setAccelerations, std::placeholders::_1, std::placeholders::_2 );
    beam::fem::restartReadData( acc, setAcc, driver );
    acc.close();
...
//------------------------------------------------------------------------------
template< typename SETFUN, typename DRIVER >
void beam::fem::restartReadData( std::istream & inp, SETFUN & setFun,
                                 DRIVER & driver )
{
    typedef typename DRIVER::Node Node;
    typedef std::function<unsigned( const Node* )> GetId;
...
void beam::fem::restartRemove( const std::string & basename,
                               const unsigned step )
{
    const std::string s = std::to_string( step );
