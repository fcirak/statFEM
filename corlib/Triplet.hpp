...
#define ALLOWED_MEM_OVERHEAD 1.e+7 // entries, this corresponds to 153 MB of 
...
public:
    ...
    IndexType nnz() const { return static_cast<IndexType> (tripletVec_.size()); }
    ...
    void finishAssembly( )
    {
        ...
        const unsigned nnz = std::distance( tripletVec_.begin( ),  itStore);
    ...
    void getIntervalCounts( const IndexType low, const IndexType high, 
                            std::vector<IndexType> & nnzRowsIn, 
                            std::vector<IndexType> & nnzRowsOut) 
    {
        // sort entries to make sure
        if ( not sorted_ ) {
            this -> finishAssembly( );
    ...
    //! Get specific entry
    void getEntry( const IndexType index, IndexType & row, 
                   IndexType & col, ValueType & val ) const
    {
...
private:
    ...
    IndexType deduceNumRows_( ) const
    {
        IndexType nr;
    ...
    struct TripletLessThan_ 
        : public boost::function<bool( const Triplet_, const Triplet_ )>
    {
        bool operator() ( const Triplet_ firstTriplet, const Triplet_ secondTriplet) const
        {
        
            const IndexPair_ firstTripletIndxPair  = firstTriplet.first;
            const IndexPair_ secondTripletIndxPair = secondTriplet.first;
    ...
    struct TripletRowLessThan_ 
        : public boost::function<bool( const Triplet_, const Triplet_) >
    {
        bool operator() ( const Triplet_ firstTriplet, const Triplet_ secondTriplet) const
        {
        
            const IndexType firstRowIndex  = firstTriplet.first.first;
            const IndexType secondRowIndex = secondTriplet.first.first;
    ...
    struct TripletRowIndexLessThan_ 
        : public boost::function<bool( const Triplet_, const IndexType) >
    {
        bool operator() ( const Triplet_ triplet, const IndexType row ) const
        {
        
            const IndexType tripletRowIndex  = triplet.first.first;
    ...
    struct TripletDiagEntry_ 
        : public boost::function<bool( const Triplet_ )>
    {
        bool operator() ( const Triplet_ triplet ) const
        {
        
            const IndexType tripletRowIndex  = triplet.first.first;
            const IndexType tripletColIndex  = triplet.first.second;
    ...
    Triplet_ combineTriplets_ ( typename TripletVec_::const_iterator itb, 
                                typename TripletVec_::const_iterator ite ) const
    {

        const IndexPair_ tmpPair = (*itb).first;
        ValueType val = 0.0;
        ...
        const Triplet_ combinedTriplet = std::make_pair(tmpPair, val); 
