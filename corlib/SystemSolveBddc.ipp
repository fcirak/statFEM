                                        ...
                                        const unsigned meshDim ) 
{
    ...
    for ( int iNode = 0; iNode < numNodesSub_ ; iNode++ ) {
        if ( meshPart.getNodePointer( iNode ) -> isDDActive( ) ) {
            numNodesLoc++;
        }
    }
    ...
    for ( int iNode = 0; iNode < numNodesSub_ ; iNode++ ) {
        ...
        for ( int i = 0; i < nDim_; i++ ) {
            ...
            xyz_[ i * numNodesSub_ + iNode ] = (meshPart.getNodePointer( iNode ) -> giveCoordinates( ))[i];
        }

        // get number of dofs at node
        nndf_[ iNode ] = static_cast<int>( meshPart.getNodePointer( iNode ) -> giveNumDofs( ) );

        // global node number
        isngn_[ iNode ] = meshPart.getNodePointer( iNode ) -> giveId( );

        // global variables - this fills isvgvn array
        std::vector<unsigned> aux( nndf_[ iNode ] );
        meshPart.getNodePointer( iNode ) -> copyDofArray( aux );
    ...
    boost::function<void( Node * )> iP = boost::bind( detail_::inetComposer<Node,Global2LocalMap_>, _1, global2LocalNodeMap, &inet_ );
    ...
    for ( int iEle = 0; iEle < numElemSub_; iEle++ ) {
        ...
        meshPart.getElementPointer( iEle ) -> iterateOverNodes( iP );
        ...
        FTL_VERIFY_DESCRIPTIVE( meshPart.getElementPointer( iEle ) -> isDDActive(),
    ...
    FTL_VERIFY_DESCRIPTIVE( inet_.size() == std::accumulate( nnet_.begin(), nnet_.end(), 0 ),
                                        ...
                                        const unsigned   meshDim ) 
{
    ...
    for ( int iEle = 0; iEle < numElemSub_; iEle++ ) {
        ...
        for ( unsigned ien = 0; ien < numSplinesPerCell; ien++ ) {
            ...
            unsigned indGlob = static_cast<int>( meshPart.getElementPointer( iEle ) -> getControlPoint( ien ) -> giveId() );
                                             ...
                                             const unsigned   meshDim ) 
{
    ...
    for ( int iEle = 0; iEle < numElemSub_; iEle++ ) {
        ...
        for ( unsigned ien = 0; ien < numSplinesPerCellP; ien++ ) {
            ...
            // get global node on element
            unsigned indGlob = static_cast<int>( meshPart.getElementPointer( iEle ) -> getControlPointP( ien ) -> giveId() );
                                           ...
                                           int verboseLevel, int  maxIt, int  ndecrMax, bool use_adaptive )
{
    ...
    for ( unsigned inz = 0; inz < a_sparse.size(); inz++ ) {
        ...
        // swap indices where they became wrong due to renumbering
        if ( onlyUpperTriangle and indRowLoc > indColLoc ) {
           i_sparse[ inz ] = indColLoc;
           j_sparse[ inz ] = indRowLoc;
        }
        else {
    ...
    std::vector<double> userConstraints(1);
    int luserConstraints1 = 0;
    int luserConstraints2 = 0;

    std::vector<double> elementData(1);
    int lelementData1 = 0;
    int lelementData2 = 0;

    std::vector<double> dofData(1);
    int ldofData = 0;
    ...
    bddcml_upload_subdomain_data( &numElem_, &numNodes_, &numDofsInt, &nDim_, &meshDim_, 
                                  ...
                                  &(elementData[0]), &lelementData1, &lelementData2,
                                  &(dofData[0]), &ldofData );
    i_sparse.clear();
    j_sparse.clear();
    a_sparse.clear();
    ...
    //============= parameters for recycling of the Krylov subspace
    int recyclingInt = 1; 
    int maxNumberOfStoredVectors = 50; 

    // call iterative solver
    bddcml_solve( & commInt, & method, & tol, & maxIt, & ndecrMax, 
                  & recyclingInt, & maxNumberOfStoredVectors,
                                            ...
                                            SubVec_ & result ) const
{
    ...
    VecUint_::const_iterator dofIter = dofIndices.begin();
    VecUint_::const_iterator dofEnd  = dofIndices.end();
    SubVec_::iterator valIter = result.begin();
