...
#ifndef del2_apps_reference_laplaceInverse_hyperParamLearning_h
#define del2_apps_reference_laplaceInverse_hyperParamLearning_h
                          ...
                          HpsValueMap & hParamMap )
{
    ...
    std::array<std::string, 6> freeHParamIds = { "gaussianProcess.w0",
                                                 "gaussianProcess.w1",
                                                 "gaussianProcess.w2",
                                                 "gaussianProcess.w3",
                                                 "gaussianProcess.w4",
    ...
    //! define the posterior density \f$ p(\theta | y) \f$
    ...
    //! the material coefficients are in log domain
    for ( unsigned i = 0; i < numFreeHParams - 1; ++ i ) {
    ...
    //! prior for \f$ \sigma_e \f$
    const unsigned idE = 5;
    const auto itMeanE = hParamMap.find( "priorMean." + freeHParamIds[ idE ] );
    FTL_VERIFY( itMeanE != hParamMap.end( ) );
    priorMean( idE ) = itMeanE->second;

    const auto itCovE = hParamMap.find( "priorCov." + freeHParamIds[ idE ] );
    FTL_VERIFY( itCovE != hParamMap.end( ) );
    priorCovar( idE, idE ) = itCovE->second;
    ...
    auto posteriorGivenData = [ & likelihoodKOH,  & priors, & hParamMap, & enrichWithPrior,
        ...
        //! transformation only valid for \sigma_e
        double likelihood = likelihoodKOH.giveLogLikelihood( hParamMap ) + hpVec( 5 );

        //! the material coefficients' prior are defined in log domain, but not \sigma_e
        VecHps hpPrior;
        hpPrior = hpVec;
        hpPrior( 5 ) = std::exp( hpVec( 5 ) );

        //! add contribution from prior when enrichWithPrior == true
        if ( enrichWithPrior ) {
            likelihood += std::log( priors.giveValue( hpPrior ) );
        }
    ...
    //! get the initial sample and proposed proposal increment
    ...
    //! First 30% of samples discarded (burn-in)
    ...
    hpsMeanStream << "# Output file containing the learned hyperparameters" << std::endl;
    for ( unsigned k = 0; k < numFreeHParams; ++ k ) {
        //! mean
