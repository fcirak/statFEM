...
int main( int argc, char* argv[] )
{
    ...
    std::string meshFile, allHyperFile, infInputFile, conductCoordFile;
    ...
    prop->registerPropertiesVar( "allHyperFile", allHyperFile );
    prop->registerPropertiesVar( "infInputFile", infInputFile );
    prop->registerPropertiesVar( "conductCoordFile", conductCoordFile );
    ...
    apps::readParamToMap( allHyperFile, hyperParams );

    //! read the training data
    Eigen::MatrixXd trainCoords;
    apps::readMatrixData( conductCoordFile, trainCoords );

    typedef eigenX::VectorSd<dim>        VecDim;
    typedef infer::ExpKernel<VecDim>     ExpKernel;

    const unsigned numConductivity = 5;
    typedef infer::GaussianProcess<ExpKernel, numConductivity> GaussProc;
    ...
    //! functor to interpolate material property at the element's centroid
    ...
    //! iterate the material function over element
    ...
    inference( numDofs, mesh, quadrature, sysmat, infInputFile, hyperParams, gaussProc );
